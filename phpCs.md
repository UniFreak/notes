# concepts
- coding strandards: PEAR, PSR-2

    is a directory with a specific sub-directory structure and a ruleset.xml file

- sniffs, category of sniffs

    All sniffs in PHP_CodeSniffer must belong to a coding standard
...
2. run `composer global config bin-dir --absolute` to make sure you can run `phpcs` in terminal
...
Usage: phpcs [-nwlsaepqvi] [-d key[=value]] [--colors] [--no-colors]
 ...
 --config-set          set global configuration options
 --config-delete       delete global configuration options
 --config-show         show global configuration options
 --runtime-set         set runtime configuration options
...
and example: <https://raw.githubusercontent.com/squizlabs/PHP_CodeSniffer/master/phpcs.xml.dist>
...
- ruleset format reference

# defining sniffs
1. create folder structure: <standard>/<category>/<sniffFile>
2. implement `register` and `process` methods
3. example: <https://github.com/squizlabs/PHP_CodeSniffer/wiki/Coding-Standard-Tutorial>
...
# codies:
- list installed coding standards: `phpcs -i`
- list a coding standars's sniffs: `phpcs --standard=PSR1 -e`

# workflow
1. phplint to make sure syntax correctness. phpcs doesn't do that
2. then run phpcs
